package yzhkof.guxi
{
	...
	public class FocusViewer extends Sprite
	{
		...
		private static const enabled_btn:TextPanel=new TextPanel(0xffff00);
...
//		private static var _stageFocus:String;
		...
		public static function init(dobj:DisplayObjectContainer, $stage:Stage):DisplayObjectContainer{
			_stageRef = $stage;
//			_stageFocus = getQualifiedClassName(_stageRef.focus);
		...
		private static function __onEnterFrame(e:Event):void{
			if(_stageRef.focus){
				if(_focusInstance != _stageRef.focus.name){
					...
					var str:String = _focusInstance + " " + getQualifiedClassName(_stageRef.focus);
					textPlus(str + "\n");
				}
			} else {
				if(_focusInstance != getQualifiedClassName(_stageRef.focus)){
					_focusInstance = "null";
					textPlus("null\n");
				}
			}
